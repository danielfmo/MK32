    ...
    if (keyboard_q != NULL) {
        {
            while (1) {
                ...
                if (xQueueReceive(keyboard_q, &key_report, portMAX_DELAY)) {
                    ...
                    hid_dev_send_report(hidd_le_env.gatt_if, hid_conn_id, HID_RPT_ID_KEY_IN, HID_REPORT_TYPE_INPUT, HID_KEYBOARD_IN_RPT_LEN, key_report);
                }
            }
        }
    } else {
    ...
    if (mouse_q != NULL) {
        {
            while (1) {
                ...
                if (xQueueReceive(mouse_q, &mouse_report, portMAX_DELAY)) {
                    // if we are not connected, discard.
                    if (sec_conn == false) continue;
                    hid_dev_send_report(hidd_le_env.gatt_if, hid_conn_id, HID_RPT_ID_MOUSE_IN, HID_REPORT_TYPE_INPUT, HID_MOUSE_IN_RPT_LEN, mouse_report);
                }
            }
        }
    } else {
    ...
    if (joystick_q != NULL) {
        {
            while (1) {
                ...
                if (xQueueReceive(joystick_q, &joystick_report, portMAX_DELAY)) {
                    // if we are not connected, discard.
                    if (sec_conn == false) continue;
                    hid_dev_send_report(hidd_le_env.gatt_if, hid_conn_id, HID_RPT_ID_JOY_IN, HID_REPORT_TYPE_INPUT, HID_JOYSTICK_IN_RPT_LEN, joystick_report);
                }
            }
        }
    } else {
    ...
    if (media_q != NULL) {
        {
            while (1) {
                ...
                if (xQueueReceive(media_q, &media_report, portMAX_DELAY)) {
                    // if we are not connected, discard.
                    if (sec_conn == false) continue;
                    hid_dev_send_report(hidd_le_env.gatt_if, hid_conn_id, HID_RPT_ID_CC_IN, HID_REPORT_TYPE_INPUT, HID_CC_IN_RPT_LEN, media_report);
                }
            }
        }
    } else {
 ...
 * */
esp_err_t halBLEEnDisable(int onoff) { return ESP_OK; }
